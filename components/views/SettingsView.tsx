...
const BackstageView: React.FC<BackstageViewProps> = ({ onNavigateToBoard }) => {
  ...
  const updatePromptStyle = (part: string, category: 'style' | 'lighting') => {
  ...
  // Get first beat for Real Preview
                ...
                <div className="flex flex-col h-full animate-in fade-in duration-300">
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col md:flex-row bg-[#0c0c0c]">
                        ...
                        <div className="w-80 overflow-y-auto border-r border-[#222] bg-[#0f0f0f]">
                            ...
                            {/* UPDATED ELEMENT SELECTOR (Vertical Visual List inside controls) */}
                                            ...
                                            <div className={`space-y-6 ${!blockBounds.enabled ? 'opacity-30 pointer-events-none' : ''}`}>
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex bg-[#111] rounded p-0.5 border border-[#333]">
                                                        <button 
                                                            onClick={() => updateBlockBounds({ mode: 'active' })}
                                                            className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded ${blockBounds.mode === 'active' ? 'bg-[#f5a623] text-black' : 'text-gray-500'}`}
                                                        >Active Focus</button>
                                                        <button 
                                                            onClick={() => updateBlockBounds({ mode: 'all' })}
                                                            className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded ${blockBounds.mode === 'all' ? 'bg-[#f5a623] text-black' : 'text-gray-500'}`}
                                                        >X-Ray All</button>
                                                    </div>
                                                </div>
            ...
            {activeCategory === 'board' && (
                <ViewContainer title="Board Layers" subtitle="Adjust the visual stacking order of elements on the board.">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="flex items-center justify-center border-2 border-dashed border-[#222] rounded-sm p-12">
                            <div className="text-center opacity-50">
                                <Layers size={64} className="mx-auto text-[#333] mb-6" />
                                <p className="text-gray-500 text-xs font-mono uppercase tracking-widest">Visual Layer Management</p>
                            </div>
                        </div>
                    </div>
                </ViewContainer>
