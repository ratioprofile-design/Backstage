...
// ... (constants A4_WIDTH, NOTE_COLORS, etc. remain unchanged)
const A4_WIDTH = 794;  
...
// ... (useDebounce and BeatEditorBlock component unchanged)
...
// ... (runPaginationPass function unchanged)
...
const ScriptView: React.FC = () => {
  ...
  // ... (Component state like searchTerm, zoom, activeBeatId, showNav, activeSidebar, scratchpadMode, draggedNoteIndex, confirmDeleteNoteId, isAnalyzing, diffVersion...)
  ...
  // ... (Memos for sortedBeats, activeBeat, uniqueLocations, uniqueCharacters)
  const sortedBeats = useMemo(() => { if (beats.length === 0) return []; return [...beats].sort((a, b) => { if (Math.abs(a.x - b.x) > 50) return a.x - b.x; return a.y - b.y; }); }, [beats]);
  ...
  useEffect(() => { handleFitZoom(); }, [activeSidebar, showNav]);
  ...
  // ... (handleAnalyzeBreakdown, addTag, removeTag, handleCreateSnapshot, handleRestoreClick, confirmRestoreVersion logic unchanged)
  ...
  // ... (Note operations: addNote, updateNote, deleteNote, dragging handlers)
  ...
  const filteredBeats = useMemo(() => { if (!searchTerm) return sortedBeats; const lower = searchTerm.toLowerCase(); return sortedBeats.filter(b => (b.slug.location || '').toLowerCase().includes(lower) || (b.content || '').toLowerCase().includes(lower) || (b.title || '').toLowerCase().includes(lower)); }, [sortedBeats, searchTerm]);
  ...
  const FORMAT_BUTTONS = [ { id: 'action', label: '1. Action', short: 'Opt+1', icon: AlignLeft }, { id: 'character', label: '2. Character', short: 'Opt+2', icon: User }, { id: 'dialogue', label: '3. Dialogue', short: 'Opt+3', icon: MessageSquare }, { id: 'parenthetical', label: '4. Paren', short: 'Opt+4', icon: Parentheses }, { id: 'transition', label: '5. Trans', short: 'Opt+5', icon: ArrowRightLeft }, { id: 'shot', label: '6. Shot', short: 'Opt+6', icon: Camera }, { id: 'lyrics', label: '7. Lyrics', short: 'Opt+7', icon: Music }, ];

  // ... (highlightSourceText, clearHighlight, handleDragStart/Over/Leave/Drop, TagInput, BreakdownSection preserved)
        ...
        <div className="w-64 bg-[#0a0a0a] border-r border-[#222] flex flex-col shrink-0 z-20 shadow-2xl transition-all">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-0.5">
                {filteredBeats.map((beat, i) => (
                    <button 
                        key={beat.id}
                        onClick={() => { const el = document.getElementById(`beat-${beat.id}`); el?.scrollIntoView({ behavior: 'smooth', block: 'center' }); setActiveBeatId(beat.id); }}
            ...
            {activeSidebar !== 'none' && (
                <div className="w-[400px] bg-[#161616] border-l border-[#333] flex flex-col animate-in slide-in-from-right-10 duration-200 z-30 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex-1 relative overflow-hidden">
                        ...
                        {/* SCRATCHPAD MODE (UPDATED WITH CONFIG PROP) */}
